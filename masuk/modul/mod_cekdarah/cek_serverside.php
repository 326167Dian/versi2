...
if ($_GET['action'] == "table_data") {
    ...
    $querycount = $db->query("SELECT count(c.id_cekdarah) as jumlah FROM cekdarah c");
    ...
    if (empty($_POST['search']['value'])) {
        $query = $db->query("SELECT c.*, p.nm_pelanggan FROM cekdarah c LEFT JOIN pelanggan p ON p.id_pelanggan = c.id_pelanggan ORDER BY $order DESC LIMIT $limit OFFSET $start");
    } else {
        $search = $db->real_escape_string($_POST['search']['value']);
        $query = $db->query("SELECT c.*, p.nm_pelanggan FROM cekdarah c LEFT JOIN pelanggan p ON p.id_pelanggan = c.id_pelanggan WHERE p.nm_pelanggan LIKE '%$search%' OR c.petugas LIKE '%$search%' OR c.gula LIKE '%$search%' OR c.asamurat LIKE '%$search%' OR c.kolesterol LIKE '%$search%' OR c.tensi LIKE '%$search%' OR c.waktu LIKE '%$search%' ORDER BY $order DESC LIMIT $limit OFFSET $start");

        $querycount = $db->query("SELECT count(c.id_cekdarah) as jumlah FROM cekdarah c LEFT JOIN pelanggan p ON p.id_pelanggan = c.id_pelanggan WHERE p.nm_pelanggan LIKE '%$search%' OR c.petugas LIKE '%$search%' OR c.gula LIKE '%$search%' OR c.asamurat LIKE '%$search%' OR c.kolesterol LIKE '%$search%' OR c.tensi LIKE '%$search%' OR c.waktu LIKE '%$search%'");
        $datacount = $querycount->fetch_array();
