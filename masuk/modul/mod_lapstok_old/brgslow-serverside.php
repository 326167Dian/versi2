...
if ($_GET['action'] == "table_data") {
    ...
    $querycount = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM trkasir_detail
    ...
    $datacount = mysqli_num_rows($querycount);
    ...
    $totalL30 = 0;
    $totalStok = 0;
    $querycount1 = $db->query("SELECT trkasir_detail.id_barang,
                    SUM(trkasir_detail.qty_dtrkasir) as q30,
                    SUM(trkasir_detail.hrgttl_dtrkasir) as om30 FROM trkasir_detail
        JOIN trkasir ON trkasir.kd_trkasir = trkasir_detail.kd_trkasir
        WHERE trkasir.tgl_trkasir BETWEEN '$_GET[start]' AND '$_GET[finish]'
    ...
    while ($value1 = $querycount1->fetch_array()) {
        $om301 = $value1['om30'];
        $q301 = $value1['q30'];

        $getbrg1 = $db->query("SELECT SUM(hrgsat_barang * stok_barang) AS nilaibarang, SUM($om301 - ($q301*hrgsat_barang)) as l30 FROM barang WHERE id_barang = '$value1[id_barang]'");
        $brg1 = $getbrg1->fetch_array();
    ...
    if (!empty($query)) {
        ...
        while ($value = $query->fetch_array()) {
            $getbrg = $db->query("SELECT * FROM barang WHERE id_barang = '$value[id_barang]' ORDER BY id_barang");
            $brg = $getbrg->fetch_array();
