...
?>
        ...
        case "tambah":
            ...
            if ($ketemucekkd > 0) {
                ...
                mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO kdbm(kd_trbmasuk,id_resto,id_admin) VALUES('$kdtransaksi','pusat','$_SESSION[id_admin]')");
            }
		  ...
		  <div class='box box-primary box-solid'>
				...
				<div class='box-body table-responsive'>
						...
						<form onsubmit='return false;' method=POST action='$aksi?module=trbmasukpbf&act=ubah_trbmasuk' enctype='multipart/form-data' class='form-horizontal'>
							   ...
							   <input type=hidden name='kd_trbmasuk1' id='kd_trbmasuk1' value='$re[kd_orders]'>
									...
									<label class='col-sm-4 control-label'>Exp. Date</label>
										<div class='col-sm-7'>
											<input type='text' class='datepicker' name='exp_date' id='exp_date' required='required' autocomplete='off'>
												...
												<div class='buttons'>
													<button type='button' class='btn btn-success right-block' onclick='simpan_ubah_detail();'>SIMPAN DETAIL</button>
												</div>
										</div>
		  ...
		  <div class='box box-primary box-solid'>
				...
				<div class='box-body table-responsive'>
						...
						<form onsubmit='return false;' method=POST action='$aksi?module=trbmasukpbf&act=ubah_trbmasuk' enctype='multipart/form-data' class='form-horizontal'>
						       ...
						       <input type=hidden name='id_trbmasuk' id='id_trbmasuk' value='$re[id_trbmasuk]'>
							   <input type=hidden name='kd_trbmasuk' id='kd_trbmasuk' value='$re[kd_trbmasuk]'>
							   <input type=hidden name='kd_trbmasuk1' id='kd_trbmasuk1' value='$kdtransaksi'>
    ...
    <?php        
        ...
        case "evaluasi_tampil":
            ...
            $trbmasuk   = $db->query("SELECT * FROM trbmasuk WHERE id_trbmasuk = '$id' AND kd_orders != ''");
            $data       = $trbmasuk->fetch_array();
    ?>
                        ...
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <?php
                                    ...
                                    $no = 1;
                                    $total = 0;
                                    while($detail = $trbmasuk_detail->fetch_array()):
                                        $subtotal = $detail['hrgsat_dtrbmasuk'] * $detail['qty_dtrbmasuk'];
                                        $total  = $total + $subtotal;
                                        $orders = $db->query("SELECT * FROM ordersdetail 
                                                                    WHERE kd_trbmasuk = '$detail[kd_orders]' AND id_barang = '$detail[id_barang]'");
                                        $order  = $orders->fetch_array();                            
                                ?>
                                <tr>
                                    ...
                                    <td align="center" <?=($order['qty_dtrbmasuk'] > $detail['qty_dtrbmasuk'])?'style="background-color:#f95959"':(($order['qty_dtrbmasuk'] < $detail['qty_dtrbmasuk'])?'style="background-color:#00bbf0"':'');?>>
                                        <?=$detail['qty_dtrbmasuk'];?></td>
