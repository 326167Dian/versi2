...
<div class="box-body table-responsive">
    <table id="example5" class="table table-condensed table-bordered table-striped table-hover">
        ...
        $carabayar = $rf['carabayar'];**/
            ...
            $cord   = mysqli_num_rows($noreq);
            if ($cord > 0) {
                while ($r = mysqli_fetch_array($noreq)) {
                    
                $brgord = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM barang WHERE kd_barang='$r[kd_barang]'");
                $bro    = mysqli_fetch_array($brgord);
            ...
            $trbmasuk = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM trbmasuk_detail
                                                                    WHERE kd_orders ='$kd_trbmasuk' ORDER BY id_dtrbmasuk ASC");
            $ctrb   = mysqli_num_rows($trbmasuk);
            if ($ctrb > 0) {
                while($trb  = mysqli_fetch_array($trbmasuk)){
                        
                        $brgmsk = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM barang WHERE kd_barang='$trb[kd_barang]'");
                        $brm    = mysqli_fetch_array($brgmsk);
    ...
    // //             $trbmasuk = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM trbmasuk_detail
    // //                                                                     WHERE kd_orders ='$r[kd_trbmasuk]' ORDER BY id_dtrbmasuk ASC");
    // //             $ctrb   = mysqli_num_rows($trbmasuk);
    ...
    // //                 $hrgjual_dtrbmasuk = format_rupiah($rbrg['hrgjual_barang']);
    // //                 $hnasat_dtrbmasuk = format_rupiah($rbrg['hna']);
    // //                 $hrgttl_dtrbmasuk = format_rupiah(($bro['hna'] * $r['qtygrosir_dtrbmasuk']) * (1-($r['diskon']/100)));
    // //                 $hnadisc = $rbrg['hna'] * (1 - ($r['diskon'] / 100));
    // //                 $hnadisc1 = format_rupiah($hnadisc);
    // //                 $totalharga[] = round(($bro['hna'] * $r['qtygrosir_dtrbmasuk']) * (1-($r['diskon']/100)));
    ...
    // //                 // $totalharga[]       = $trb['hrgttl_dtrbmasuk'];
    // //                 // $totalharga[]       = ($trb['hnasat_dtrbmasuk'] * $trb['qty_grosir']) * (1-($trb['diskon']/100));
    // //                 $totalharga[]       = round($brm['hna'] * $trb['qty_grosir']) * (1-($trb['diskon']/100));
