...
if ($_GET['action'] == "table_data") {
    ...
    $querycount = $db->query("SELECT count(id_shift) as jumlah FROM waktukerja");
    ...
    if (empty($_POST['search']['value'])) {
        $query = $db->query("SELECT w.*, n.nama_shift FROM waktukerja w LEFT JOIN namashift n ON n.shift = w.shift ORDER BY $order DESC LIMIT $limit OFFSET $start");
    } else {
        $search = $db->real_escape_string($_POST['search']['value']);
        $query = $db->query("SELECT w.*, n.nama_shift FROM waktukerja w LEFT JOIN namashift n ON n.shift = w.shift WHERE w.petugasbuka LIKE '%$search%' OR w.petugastutup LIKE '%$search%' OR w.tanggal LIKE '%$search%' OR n.nama_shift LIKE '%$search%' OR w.status LIKE '%$search%' ORDER BY $order $dir LIMIT $limit OFFSET $start");

        $querycount = $db->query("SELECT count(w.id_shift) as jumlah FROM waktukerja w LEFT JOIN namashift n ON n.shift = w.shift WHERE w.petugasbuka LIKE '%$search%' OR w.petugastutup LIKE '%$search%' OR w.tanggal LIKE '%$search%' OR n.nama_shift LIKE '%$search%' OR w.status LIKE '%$search%'");
        $datacount = $querycount->fetch_array();
